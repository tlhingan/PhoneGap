...
FSR.properties = {
    ...
    invite : {
        ...
        */
        ...
        exclude : {
            local : ['/getinvolved/Pages/home.aspx','/getinvolved/pages/home.aspx'],
            referrer : []
        },
        include : {
            local : [ '.' ]
...
if (typeof $.parseJSON(FSR.Cookie.read('fsr.s')).cp == 'undefined') {
	FSR.CPPS.set('gsaQuery','none');
}
if (typeof page_query != 'undefined') {
	var gsaQueries = $.parseJSON(FSR.Cookie.read('fsr.s')).cp.gsaQuery;
	if (gsaQueries == "none" && page_query != null ) {
		FSR.CPPS.set('gsaQuery',page_query);
	}
	if (gsaQueries != "none" && page_query != null ) {
		if (gsaQueries.length > 1000) {
			FSR.CPPS.set('gsaQuery',page_query);
		} else {
			FSR.CPPS.set('gsaQuery',gsaQueries+","+page_query);
		}
	}
}