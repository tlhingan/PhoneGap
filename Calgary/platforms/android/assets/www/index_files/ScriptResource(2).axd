...
CocisUtilities = function () {

    // Parses out query string using JavaScript
    ...
    this.dcsAction = function (creators, title, action) {
...
cocisUtilities = new CocisUtilities();




/* Page initialization region */

// Initialization tasks that start right after the document finishes loading
$(document).ready(function () {
    setupScrolling();
    new SearchBox().Initialize();
    new PopularSearchbox().Initialize();
    searchHint.Initialize();
    floatingFooter.Initialize();
});


// Sets up the page for scrolling. Undoes the transfer of scrolling from <body> to <div id="s4-workspace">
// if the ribbon is not visible.
function setupScrolling() {
    // Test if the ribbon is visible
    if ($("#s4-ribbonrow").length == 0) {
        $("#s4-workspace")
			.attr("id", "s4-workspace-no-scroll");
        $("body")
			.addClass("cocis-yes-scroll")
			.attr("scroll", "yes");
    }
}
...
SearchBox = function () {
    ...
    this.Initialize = function () {
        ...
        searchBox.focus(function () {
            searchBox.autocomplete({
                source: function (request, response) {
                    ...
                    var searchText = $("#ctl00_PlaceHolderSearchArea_ctl00_txtSearch").val();

                    $.ajax({
                        url: "/_layouts/cocis/GSASuggest.ashx",
				...
				select: function(event, ui) {
					if(ui.item){
						searchBox.val(ui.item.value);
							...
							ga('send', 'event', 'query suggestion', ui.item.value );
        ...
        // This also triggers the attachment of the autocomplete plugin for the first time.
...
/* Popular search region */

PopularSearchbox = function () {

    this.popularTermsOpen = false;

    var _self = this;

    // Sets up the popular items dropdown
    this.Initialize = function () {
        $("#cocis-popular-search-terms ul.cocis-list-horiz a").click(_self.togglePopularTerms);
        $("#cocis-popular-search-terms-list .cocis-body ul li a").click(_self.hidePopularTerms);

        // Hide the popular terms list if it loses focus.
        $("body").focusin(_self.hidePopularTermsConditional)
    		     .click(_self.hidePopularTermsConditional);
    }

    // Hide the popular terms drop down if none of them are in focus.
    this.hidePopularTermsConditional = function () {
        if (_self.popularTermsOpen) {
            // If the currently focused element is not part of the popular terms list.
            var isPopularTerms = $(document.activeElement).parents().is("#cocis-popular-search-terms");
            if (isPopularTerms == false) {
                _self.hidePopularTerms();
            }
        }
    }

    // Toggles the popular search terms
    this.togglePopularTerms = function () {
        if (_self.popularTermsOpen) {
            _self.hidePopularTerms();
            $("#cocis-popular-search-terms ul.cocis-list-horiz a").first().focus();
        } else {
            _self.showPopularTerms();
        }
    }

    // Shows the popular search terms
    this.showPopularTerms = function () {
        $("#cocis-popular-search-terms ul.cocis-list-horiz a").html("Hide popular search terms");
        $("#cocis-popular-search-terms ul.cocis-list-horiz li").css("background-position", "-227px -119px");
        $("#cocis-popular-search-terms-list").slideDown(200, function () {
            _self.popularTermsOpen = true;
            $("#cocis-popular-search-terms-list li a").first().focus();
        });
    }

    // Hides the popular search terms
    this.hidePopularTerms = function () {
        $("#cocis-popular-search-terms ul.cocis-list-horiz a").html("Show popular search terms");
        $("#cocis-popular-search-terms ul.cocis-list-horiz li").css("background-position", "-227px -106px");
        $("#cocis-popular-search-terms-list").slideUp(200, function () {
            _self.popularTermsOpen = false;
        });
    }
}
...
SearchHint = function () {
    ...
    this.Initialize = function () {
        ...
        _self.searchBox = $("#ctl00_PlaceHolderSearchArea_ctl00_txtSearch");

        _self.searchBox.focus(function () {
            _self.hideSearchHint();
        }).blur(function () {
            _self.showSearchHint();
        });

        // Show the hint text if this is not the home page.
        // On the home page the initial focus is on the search input.
        // This code is so specific because document.activeElement is not universally supported 
        // so we cannot easily tell if the search input has focus and thus whether or not we should
        // initially show the hint text.
        if ($("body.cocis-homepage").length == 0) {
            _self.showSearchHint();
        } else {
            _self.hideSearchHint();
        }
    ...
    // Display the search hint
    this.showSearchHint = function () {
        if (_self.searchBox.val() === "") {
            _self.searchBox.css("color", "#ccc");
            _self.searchBox.val(_self.searchHint);
        }
    }

    // Hide the search hint
    this.hideSearchHint = function () {
        if (_self.searchBox.val() === _self.searchHint) {
            _self.searchBox.val("");
            _self.searchBox.css("color", "#000");
        }
    }
}
...
/* Floating footer region */

FloatingFooter = function () {
    this.offset;
    this.isMapping;

    // How frequently the floating property should be checked.
    // Part of the refactoring to make this feature more performant in IE7.
    this.delay = 50;
    this.shouldCheck = true;
    this.finalCheckTimer;

    var _self = this;

    this.Initialize = function () {

        // This doesn't support any IE browsers under 7
        // or iphone / ipad
        ...
        // 6px should be the distance between the top of the footer row and the bottom of the floating footer’s proper position
        _self.offset = $('#cocis-footer-row').offset().top - 6;

        var isMapping = $("body").hasClass("cocis-mapping");
        _self.checkFloat();

        // To get floating footer working on mapping page, watch for scrolling
        // on window. Otherwise, SharePoint sets scrolling on page to occur on
        // the div with id="s4-workspace". We will only keep the scrolling on
        // the div with id="s4-workspace" when the ribbon is showing.
        if (isMapping || $("#s4-ribbonrow").length == 0) {
            $(window).scroll(function () {
                _self.checkFloatSparse();
            });
        } else {
    ...
    // Sets whether the floating footer should be floating or not.
    // The frequence with which it checks, is determined by the
    // 'delay' parameter.
    this.checkFloatSparse = function () {

        if (_self.shouldCheck) {

            // Prevent the checking of this too frequently so that the performance
            // of this feature doesn't bog down IE7.
            _self.checkFloat();
            _self.shouldCheck = false;
            setTimeout("floatingFooter.shouldCheck = true;", _self.delay);

            // Make sure the check happens at least one final time after the 
            // scrolling has stopped so that it gets updated when the user 
            // scrolls really fast.
            if (_self.finalCheckTimer) {
                clearTimeout(_self.finalCheckTimer);
            }
            _self.finalCheckTimer = setTimeout("floatingFooter.checkFloat();", _self.delay + 10);
        }
    }

    // This will force a check of whether the floating footer should float or not.
    this.checkFloat = function () {
        if (($(window).scrollTop() + $(window).height()) < _self.offset) {
            $('#cocis-floating-footer').addClass('cocis-footer-loose');
            $('#cocis-floating-footer').removeClass('cocis-footer-fixed');
            $('#cocis-floating-footer-placeholder').addClass('cocis-footer-loose');
            $('#cocis-floating-footer-placeholder').removeClass('cocis-footer-fixed');
        }
        else {
            $('#cocis-floating-footer').removeClass('cocis-footer-loose');
            $('#cocis-floating-footer').addClass('cocis-footer-fixed');
            $('#cocis-floating-footer-placeholder').removeClass('cocis-footer-loose');
            $('#cocis-floating-footer-placeholder').addClass('cocis-footer-fixed');
        }
    }
}

floatingFooter = new FloatingFooter();

//shows loading icon and focuses screen on loading image for accessibility
function showLoading() {

    $("#loadingImage").remove();
    var content = "";

    var top = Math.round($(window).height() / 2) - 50 + $(window).scrollTop();
    var left = Math.round($(window).width() / 2) - 128 + $(window).scrollLeft();

    content += "<img id='loadingImage' src='/Style Library/cocis/images/loading.gif' style='position:absolute;top:" + top + "px;left:" + left + "px;z-index:99999;'/>";
    $("body").append(content);
}

//hides loading icon and sets focus on ID provided
function hideLoading(focusID) {
...
function createNotification(type, title, subtitle, description, callbackFunction) {
    $("#cocis-modal").remove();
    $("#cocis-popup").remove();

    //revert all modal requests to popup if calling from IE7 (modal too slow for IE7)
    if (type == "modal" && (($.browser.msie && $.browser.version != "7.0") || $.browser.msie == null)) {
        var content = "";
        content += "<div id='cocis-modal' class='cocis-dialog-content cocis-modal' " +
    	"title='" + title + "'" +
    	"subtitle='" + subtitle + "'>" +
    	description +
    	"</div>";
        $("body").append(content);
        setupModal($("#cocis-modal"), callbackFunction);
        showDialog($("#cocis-modal"))
    }
    else if (type == "popup" || (type == "modal" && $.browser.msie && $.browser.version == "7.0")) {
        var content = "";
        content += "<div id='cocis-popup' class='cocis-dialog-content' " +
    	"title='" + title + "'" +
    	"subtitle='" + subtitle + "'>" +
    	description +
    	"</div>";
        $("body").append(content);
        setupDialog($("#cocis-popup"), callbackFunction);
        showDialog($("#cocis-popup"))

        //overrides necessary to work with sharepoint
        $("#cocis-popup").css("position", "relative");

        //required for pop-up restores to work (since content is dynamically inserted)
        $(".cocis-dialog").data("originalWidth", $(".cocis-dialog").width());
        $(".cocis-dialog").data("originalHeight", $(".cocis-dialog").height());
    }
}

function hideNotification(focusID) {
    $("#cocis-modal").dialog("close");
    $("#cocis-popup").dialog("close");
...
function showDialog(element) {
    ...
    var dWindow = element.parent();
    centerWindow(dWindow);
}

function centerWindow(dWindow) {
    var windowHeight = $(window).height();
    var windowWidth = $(window).width();

    var height = dWindow.height();
    var width = dWindow.width();

    var top;

    //currently there is possibility of height overflowing past window, width doesn't have this issue
    if (windowHeight > height) {
        //small height adjustment for mapping
        top = Math.round((windowHeight / 2) - (height / 2) - 100);
        top = Math.min(top, 50);
    }
    else {
        top = 50;
    ...
    var left = Math.round((windowWidth / 2) - (width / 2));

    dWindow.css("top", top + "px");
    dWindow.css("left", left + "px");
}

function maximize(dWindow) {
    if (dWindow.hasClass("maximized")) {
        dWindow.css("width", dWindow.data("originalWidth") + "px");
        dWindow.css("height", dWindow.data("originalHeight") + "px");
        dWindow.css("position", "fixed");
        dWindow.removeClass("maximized");

        centerWindow(dWindow);
    }
    else {
        dWindow.addClass("maximized");
    }
}

function setupDialog(element, callbackFunction) {
    var width = element.width();
    var height = element.height();
    if (callbackFunction == null) {
        element.dialog(
		{
		    width: 500, minHeight: 200, minWidth: 200, autoOpen: false,
		    open: function () {
		        //remove inline block style for printing purposes
		        $("#cocis-popup-window").css("display", "");
		    },
		    resizeStop: function (event, ui) {
		        $(".cocis-dialog").css("position", "fixed");
		    }
		});
    }
    else {
    ...
    element.prepend("<div class=\"cocis-subtitle\">" + element.attr("subtitle") + "</div>");

    var dWindow = element.parent();
    var dTitleBar = dWindow.find(".ui-dialog-titlebar");
    var newParentID = element.attr("id") + "-window";

    dTitleBar.append("<a onclick=\"maximize($('#" + newParentID + "'));\" class=\"cocis-maximize\">Maximize</a>")

    dWindow.append("<div class=\"cocis-edge-s\"><div class=\"cocis-edge-decoration\"></div></div>");
    dWindow.append("<div class=\"cocis-edge-n\"><div class=\"cocis-edge-decoration\"></div></div>");
    dWindow.attr("id", newParentID);
    dWindow.addClass("cocis-dialog");
    dWindow.css("position", "fixed");
    dWindow.data("originalWidth", width);
    dWindow.data("originalHeight", height);

    var buttons = dWindow.find(".cocis-maximize, .ui-dialog-titlebar-close");
    buttons.mousedown(function () {
        $(this).addClass("cocis-mouse-down");
    });
    buttons.mouseup(function () {
        $(this).removeClass("cocis-mouse-down");
    });
}

function setupModal(element, callbackFunction) {
    ...
    if (callbackFunction == null) {
        element.dialog({
            width: 500, modal: true, autoOpen: false, draggable: false, resizable: false,
    ...
    dWindow.append("<div class=\"cocis-edge-s\"><div class=\"cocis-edge-decoration\"></div></div>");
    dWindow.append("<div class=\"cocis-edge-n\"><div class=\"cocis-edge-decoration\"></div></div>");
    dWindow.append("<div class=\"cocis-edge-e\"></div>");
    dWindow.append("<div class=\"cocis-edge-w\"></div>");
    dWindow.append("<div class=\"cocis-corner-ne\"></div>");
    dWindow.append("<div class=\"cocis-corner-nw\"></div>");
    dWindow.append("<div class=\"cocis-corner-se\"></div>");
    dWindow.append("<div class=\"cocis-corner-sw\"></div>");
    dWindow.attr("id", newParentID);
    dWindow.addClass("cocis-dialog");
    dWindow.addClass("cocis-modal");
    dWindow.css("position", "fixed");
}
...
$(document).ready(function () {
    $.getJSON('/_layouts/cocis/services/RssFeedAggregator.ashx', function (data) {
        newsTicker.SetNewsTickerJson(data);
        newsTicker.Initialize({ "autoTransitionIntervalLength": 6000 });
    });

});

function NewsTicker() {
    this.newsTickerIndex = 0; // the index of the current news item being displayed
    this.allJsonNewsItems = []; // holds the collection of news item objects
    this.allListNewsItems = []; // holds the collection of <li> DOM objects that comprise the list of news items
    this.autoTransitionInterval;
    this.autoTransitionIntervalLength = 5000; // default transition length for news ticker items.

    var _self = this;

    this.Initialize = function (options) {
        if (options.autoTransitionIntervalLength) {
            _self.autoTransitionIntervalLength = options.autoTransitionIntervalLength;
        }

        // Hide the news ticker if there is no news.
        if (!(_self.allJsonNewsItems instanceof Array) || _self.allJsonNewsItems.length == 0) {
            $("#cocis-news-ticker-container").hide();
        } else {
            // Build the 2 <ul>'s of news items based upon the JSON list of news items.
            $("#cocis-all-news-container").show(); // Show for the ellipsis calculations.
            $(_self.allJsonNewsItems).each(function () {
                var itemLink = this.Links[0].Uri;
                var itemText = this.Title.Text;

                // IE bug renders "Charges Laid in Nighttimes B&E" to "Charges Laid in Nighttimes B".
                // encode & to html/xml format
                if ($.browser.msie) {
                    itemText = itemText.replace(/&/g, '&amp;');
                }

                // Build news ticker list
                var newsTickerItem = $("<a>")
                    .attr("href", itemLink)
                    .attr("target", "_blank")
                    .text(itemText);
                var newsTickerListItem = $("<li>").append(newsTickerItem);
                $("#cocis-news-items ul")
                    .append(newsTickerListItem);
                _self.allListNewsItems.push(newsTickerListItem);
                newsTickerListItem.show(); // Show for the ellipsis calculations.
                _self.autoEllipsisText(newsTickerItem, 600);
                newsTickerListItem.hide(); // Hide again, ellipsis calculations complete.
                ...
                var allNewsItem = $("<a>")
                    ...
                    .attr("target", "_blank")
                    .text(itemText);
                var allNewsListItem = $("<li>").append(allNewsItem);
                $("#cocis-all-news-items .viewport .overview ul")
                    .append(allNewsListItem);
                _self.autoEllipsisText(allNewsItem, $("#cocis-all-news-items .overview").width() - 30);
            });
            $("#cocis-all-news-container").hide(); // Hide again, ellipsis calculations complete.

            // Show the first item.
            _self.SetRandomNewsTickerIndex();
            _self.allListNewsItems[_self.newsTickerIndex].show();
            // Set up news to rotate if there is more than one news item
            if (_self.allJsonNewsItems instanceof Array && _self.allJsonNewsItems.length > 1) {
                _self.autoTransitionInterval = window.setInterval(_self.RotateNews, _self.autoTransitionIntervalLength);
            }

            // Set up the scroll bar on all the news items.
            // Note: this must happen when the cocis-all-news-container is not hidden because 
            // when it is hidden it has 0 height and the tinyscrollbar plugin cannot do 
            // it's calculations properly.
            $("#cocis-all-news-container").show();
            $("#cocis-all-news-items").tinyscrollbar();
            $("#cocis-all-news-container").hide();

            // Set up the buttons.
            $("#cocis-news-pause").click(_self.PauseNews);
            $("#cocis-news-play").click(_self.PlayNews);
            $("#cocis-news-view-all").click(_self.ShowAllNews);
            $("#cocis-all-news-container .cocis-close-button a").click(_self.HideAllNews);
        }
    }

    // Trims the text in a given element so that it is no wider
    // than the given width element. Used to keep the text on
    // one line.
    this.autoEllipsisText = function (textElement, width) {
        var text = textElement.text();
        textElement.html('<span id="ellipsisSpan" style="white-space:nowrap;">' + text + '</span>');
        var textSpan = $('#ellipsisSpan');

        // If the text is too long.
        if (textSpan.width() > width) {

            // Remove words until the text is short enough to fit without line breaks.
            textSpan.html(text + '...');
            while (textSpan.width() > width) {
                text = _self.removeLastWord(text);
                textSpan.html(text + '...');
            }

            textElement.html(textSpan.text());
        } else {
    ...
    this.removeLastWord = function (text) {
        var wordDelimiters = [' ', ',', '.'];
        var lastWordIndex = -1;
        ...
        // Determine which delimiter results in the smallest word at the end
        $.each(wordDelimiters, function (i, currentDelimiter) {
            var tempWordIndex = text.lastIndexOf(currentDelimiter);
            if (tempWordIndex != -1) {
                if (tempWordIndex > lastWordIndex) {
                    lastWordIndex = tempWordIndex;
                }
            }
        });

        // Return text minus the last word
        if (lastWordIndex > 0) {
            return $.trim(text.substring(0, lastWordIndex));
        } else { // Only one word passed in
            return '';
        }
    }

    // Picks a random news item index.
    this.SetRandomNewsTickerIndex = function () {
        var randomNumber = Math.floor(Math.random() * _self.allJsonNewsItems.length);
        _self.newsTickerIndex = randomNumber;
    }

    // Called by RssFeedAggregator.ashx JavaScript with JSON as argument:
    this.SetNewsTickerJson = function (items) {
        _self.allJsonNewsItems = items; // keep this collection
    }

    // Show the next news story
    this.RotateNews = function () {
        _self.allListNewsItems[_self.newsTickerIndex].fadeOut(function () {
            _self.newsTickerIndex = (_self.newsTickerIndex + 1) % _self.allListNewsItems.length;
            _self.allListNewsItems[_self.newsTickerIndex].fadeIn();
        });
    }

    this.PauseNews = function () {
        window.clearInterval(_self.autoTransitionInterval);
        _self.autoTransitionInterval = null;
        $("#cocis-news-play").show().focus();
        $("#cocis-news-pause").hide();
    }

    this.PlayNews = function () {
        _self.autoTransitionInterval = window.setInterval(_self.RotateNews, _self.autoTransitionIntervalLength);
        $("#cocis-news-play").hide();
        $("#cocis-news-pause").show().focus();
    }

    this.ShowAllNews = function () {
        $("#cocis-all-news-container").fadeIn(200, function () {
            $("#cocis-all-news-items li a").first().focus();
        });
        $("#cocis-news-ticker-container").hide();
        _self.PauseNews();
    }

    this.HideAllNews = function () {
        //$("#cocis-all-news-container").fadeOut(200); For some reason this doesn't work...
        $("#cocis-all-news-container").hide(); // ... but this does.
        $("#cocis-news-ticker-container").fadeIn(100, function () {
            $("#cocis-news-view-all").first().focus();
        });
        _self.PlayNews();
    }
...
function configureExternalLinkDisclaimer() {
    var links = $.find("a.cocis-rte-Style-Disclaimer");
    $(links).each(function () {
        $(this).click(function () {
            var options = SP.UI.$create_DialogOptions();
            options.url = "/_layouts/cocis/ExternalLinkDisclaimer.aspx?ReturnUrl=" + this.href;
            options.dialogReturnValueCallback = Function.createDelegate(
                        this, externalLinkDisclaimer_Callback);
...
function externalLinkDisclaimer_Callback(result, value) {
    if (result == 1 && this.href != null && this.href != '') {
        window.location = this.href;
    }
}

$(window).load(function () {
    configureExternalLinkDisclaimer();
});
